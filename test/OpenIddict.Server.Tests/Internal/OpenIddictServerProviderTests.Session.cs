...
namespace OpenIddict.Server.Internal.Tests
{
    public partial class OpenIddictServerProviderTests
    {
        ...
        {
            // Arrange
            var app = new OpenIddictApplication();

            var manager = CreateApplicationManager(instance =>
            {
                instance.Setup(mock => mock.FindByPostLogoutRedirectUriAsync("http://www.fabrikam.com/path", It.IsAny<CancellationToken>()))
                    .ReturnsAsync(ImmutableArray.Create(new [] {app }));

                instance.Setup(mock => mock.HasPermissionAsync(app, OpenIddictConstants.Permissions.Endpoints.Logout, It.IsAny<CancellationToken>())).ReturnsAsync(false);
            });
            ...
            Mock.Get(manager).Verify(mock => mock.HasPermissionAsync(app, OpenIddictConstants.Permissions.Endpoints.Logout, It.IsAny<CancellationToken>()), Times.Once());
        }
