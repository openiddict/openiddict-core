...
- **Closed in this fork:** JAR (server + client), CIBA poll mode (server + client), automatic key management.
- **Largest remaining gaps:** logout and sessions (back-channel logout, session management), DPoP, dynamic client registration, SAML, and BFF/dynamic providers.
...
|---|---|---|---|
| 1 | P5 DPoP | L | One DB write per protected request for replay protection; proxy `htu` normalization |
| 2 | P12 JWT introspection | S | — |
| 3 | P11.2 dynamic providers | M | Client registration lifecycle refactor |
| 4 | P11.3 BFF | M | YARP dependency (approved) |
| 5 | P11.4 / P11.5 templates, admin API | M | New template pack (approved) |
| 6 | P11.6 SAML | XL | New package; XML signature attack surface |
