...
| SAML 2.0 IdP | ✅ built-in (v8) | 🟢 `OpenIddict.Server.Saml` + `.AspNetCore` (metadata, SP-initiated SSO Redirect/POST, signed assertions, IdP-initiated opt-in); no SLO | Done (P11.6 MVP); SLO after P1 |
...
| P11.6 SAML IdP | `b6ba3f30` | New packages `OpenIddict.Server.Saml` (net48 + net10.0): `UseSaml()`, `OpenIddictServerSamlService`, `IOpenIddictServerSamlServiceProviderStore`, `IOpenIddictServerSamlAssertionProvider`; `OpenIddict.Server.Saml.AspNetCore`: `UseAspNetCore()`, `MapOpenIddictSamlEndpoints()` (`/saml/metadata`, `/saml/sso`). | Signed AuthnRequests required by default; single root signature, RSA-SHA2 only, no DTD; ACS allow-list; challenge/callback with a data-protected state; ASP.NET Core only; no SLO/encryption/artifact binding. |
