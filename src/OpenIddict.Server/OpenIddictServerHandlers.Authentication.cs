    ...
    public static partial class Authentication
    {
        ...
        public sealed class ValidateRequestUriParameter : IOpenIddictServerHandler<ValidateAuthorizationRequestContext>
        {
            ...
            public ValueTask HandleAsync(ValidateAuthorizationRequestContext context)
            {
                ...
                if (!context.Request.RequestUri.StartsWith(RequestUris.Prefixes.Generic, StringComparison.Ordinal))
                {
                    ...
                    // When pushed authorization requests are globally required, the authorization server
                    // only accepts authorization request data via the pushed authorization endpoint
                    // (RFC 9126, section 5): external request_uri values MUST be rejected in this case.
                    if (context.Options.RequirePushedAuthorizationRequests)
                    {
                        context.Logger.LogInformation(6735, SR.GetResourceString(SR.ID6735), context.Request.RequestUri);

                        context.Reject(
                            error: Errors.InvalidRequestUri,
                            description: SR.FormatID2466(Parameters.RequestUri),
                            uri: SR.FormatID8000(SR.ID2466));
        ...
        public sealed class ResolveRequestObjectReference : IOpenIddictServerHandler<ValidateAuthorizationRequestContext>
        {
            ...
            public async ValueTask HandleAsync(ValidateAuthorizationRequestContext context)
            {
                ...
                // Note: the hash is computed over the file contents as returned by the fetcher (i.e before trimming).
                ...
                context.Request.Request = content.Trim();
