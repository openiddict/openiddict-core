...
public sealed class OpenIddictServerOptions
{
    ...
    /// <summary>
    /// Gets or sets a boolean indicating whether request objects can be passed by reference using an
    /// external HTTPS "request_uri" parameter (RFC 9101, section 5.2 and OpenID Connect Core, section 6.2).
    /// Request objects are retrieved using the registered <see cref="IOpenIddictServerRequestObjectFetcher"/>.
    /// </summary>
    /// <remarks>
    /// Note: this option requires enabling request object support using <see cref="EnableRequestObjectSupport"/>.
    /// Request URIs generated by OpenIddict for pushed authorization requests are not affected by this option.
    /// </remarks>
    public bool EnableRequestObjectReferenceSupport { get; set; }

    /// <summary>
    /// Gets or sets a boolean indicating whether external "request_uri" values must be pre-registered by
    /// the client applications using the <see cref="Settings.RequestObject.RequestUris"/> setting.
    /// This option is enabled by default and is returned as "require_request_uri_registration" in discovery.
    /// </summary>
    public bool RequireRequestUriRegistration { get; set; } = true;
    ...
    /// </summary>
    /// <remarks>
    /// Note: the JWT is generated by a handler of the ApplyAuthorizationResponseContext event whose order is 249_000.
    /// Custom handlers adding parameters to the authorization response must use a lower order for these parameters
    /// to be included in the JWT (parameters added later are not protected and are ignored by conforming clients).
    /// </remarks>
