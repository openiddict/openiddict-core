...
public static partial class OpenIddictClientHandlers
{
    public static class Introspection
    {
        ...
        /// </summary>
        public sealed class ValidateIntrospectionResponseToken : IOpenIddictClientHandler<HandleIntrospectionResponseContext>
        {
            private readonly IOpenIddictClientDispatcher _dispatcher;

            public ValidateIntrospectionResponseToken(IOpenIddictClientDispatcher dispatcher)
            ...
            public async ValueTask HandleAsync(HandleIntrospectionResponseContext context)
            {
                ...
                if (string.IsNullOrEmpty(context.IntrospectionResponseToken))
                {
                    ...
                    if (context.Registration.RequireJsonWebTokenIntrospectionResponses && string.IsNullOrEmpty(context.Response.Error))
                    {
                        context.Reject(
                ...
                if (string.IsNullOrEmpty(context.Registration.ClientId) ||
                    !principal.GetClaims(Claims.Audience).Contains(context.Registration.ClientId, StringComparer.Ordinal))
                {
                    context.Reject(
                        error: Errors.ServerError,
